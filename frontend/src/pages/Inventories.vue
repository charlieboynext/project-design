    ...
    <el-card shadow="never" class="panel">
      <div class="panel-header">
        ...
        <div class="panel-actions">
          ...
          <el-button type="primary" @click="goCreate">新增库存</el-button>
        </div>
      </div>
...
import { fetchInventories, deleteInventory } from '../api/backend'
...
async function load() {
  loading.value = true
  const res = await fetchInventories(keyword.value.trim(), page.value, size.value)
  inventories.value = res.items
...
function handleEdit(row: Inventory) {
  router.push(`/inventories/${row.id}/edit`)
}

async function handleDelete(row: Inventory) {
  try {
    await ElMessageBox.confirm(
      `确定要删除库存记录 "${row.traceCode}" 吗？`,
      '确认删除',
      {
